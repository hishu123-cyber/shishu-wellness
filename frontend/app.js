...
async function renderConstitution(){var h=hd('<i class="fa-solid fa-spa"></i> 体质测评');var body='<div class="page"><div class="card"><div class="card-title">中医九种体质</div><div class="card-subtitle">平和质·气虚质·阳虚质·阴虚质·痰湿质·湿热质·血瘀质·气郁质·特禀质</div><p style="font-size:13px;line-height:1.6">通过45道题目判断你的体质类型。</p><button class="btn btn-primary btn-block mt-4 btn-lg" onclick="navigate(\'constitution-assess\')">开始测评</button></div><div class="card-title">历史记录</div><div id="c-history"></div></div>';document.getElementById('app').innerHTML=h+body+try{var records=await api('/constitution/records');var c=document.getElementById('c-history');if(!records.length){c.innerHTML='<div style="padding:20px;text-align:center;color:var(--text2)">暂无记录</div>';return;}var html='';for(var i=0;i<records.length;i++){var r=records[i];html+='<div class="card flex-between"><span style="font-weight:600">'+r.result_type+'</span><span style="font-size:12px;color:var(--text2)">'+(r.created_at?r.created_at.slice(0,10):'')+'</span></div>';}c.innerHTML=html;}catch(e){}}
...
async function renderTea() {
  ...
  try {
    ...
    document.getElementById("tea-page").innerHTML = '<div style="padding:40px;text-align:center;color:var(--red)">加载失败<br><button class="btn btn-sm btn-outline mt-2