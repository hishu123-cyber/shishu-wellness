...
function navigate(p,params){if(!params)params={};state.page=p;state.pageParams=params;window.scrollTo(0,0);render();}
function render(){if(!state.user&&state.token){api('/auth/me').then(function(u){state.user=u;render();}).catch(function(){state.token=null;localStorage.removeItem('token');render();});document.getElementById('app').innerHTML='<div class="loading"><div class="spinner"></div><p>加载中...</p></div>';return;}if(!state.user){renderLogin();return;}var ps={home:renderHome,diary:renderDiary,tea:renderTea,shop:renderShop,'shop-product':renderShopProduct,'shop-cart':renderShopCart,'diary-edit':renderDiaryEdit,constitution:renderConstitution,'constitution-assess':renderConstitutionAssess,'constitution-result':renderConstitutionResult,recipes:renderRecipes,'recipe-detail':renderRecipeDetail,solar:renderSolar,articles:renderArticles,'article-detail':renderArticleDetail,profile:renderProfile,'profile-edit':renderProfileEdit,tcm:renderTCM,consulting:renderConsulting};var fn=ps[state.page];if(fn)fn();else renderHome();}
function nav(a){var is=[{id:'home',icon:'<i class="fa-solid fa-house"></i>',label:'首页'},{id:'diary',icon:'<i class="fa-solid fa-pen-to-square"></i>',label:'日记'},{id:'recipes',icon:'<i class="fa-solid fa-utensils"></i>',label:'食疗'},{id:'solar',icon:'<i class="fa-solid fa-sun"></i>',label:'节气'},{id:'tea',icon:'<i class="fa-solid fa-leaf"></i>',label:'茶养'},{id:'shop',icon:'<i class="fa-solid fa-store"></i>',label:'商城'},{id:'profile',icon:'<i class="fa-solid fa-user"></i>',label:'我的'}];var h='<div class="bottom-nav">';for(var i=0;i<is.length;i++){var x=is[i];h+='<button class="nav-item'+(x.id===a?' active':'')+'" onclick="navigate(\''+x.id+'\')"><span class="icon">'+x.icon+'</span>'+x.label+'</button>';}h+='<button class="nav-item" onclick="toggleTheme()"><span class="icon" id="nti"><i class="fa-solid fa-moon"></i></span>主题</button></div>';return h;}
...
async function renderHome(){updateThemeIcon();var nick=esc(state.user.nickname||state.user.username);
...
'</div>';
document.getElementById('app').innerHTML=hd+body+nav('home');
try{var results=await Promise.all([api('/diary/today'),api('/diary')]);var t=results[0];var list=results[1];
...
async function renderShop(cat){updateThemeIcon();try{
  ...
  var h='<div class="header"><h1><i class="fa-solid fa-store"></i> 养生商城</h1><p>精选养生好物</p></div>'+
    '<div class="page" style="padding-bottom:80px">'+
    // 分类导航条
    '<div class="shop-cats" style="display:flex;gap:6px;overflow-x:auto;padding:8px 16px;white-space:nowrap;-webkit-overflow-scrolling:touch;scrollbar-width:none">';
  for(var ci=0;ci<cats.length;ci++){
    var cc=cats[ci];
    h+='<span class="shop-cat-tag '+(cc===cat||(!cat&&cc==='全部')?'active':'')+'" onclick="renderShop(\''+cc+'\')" style="display:inline-block;padding:4px 14px;border-radius:20px;font-size:13px;background:'+(cc===cat||(!cat&&cc==='全部')?'var(--green)':'var(--card)')+';color:'+(cc===cat||(!cat&&cc==='全部')?'#fff':'var(--text)')+';border:1px solid '+(cc===cat||(!cat&&cc==='全部')?'var(--green)':'var(--border)')+';cursor:pointer;flex-shrink:0">'+cc+'</span>';
  }
  h+='</div>'+
    // 商品网格（2列）
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:4px 12px 12px">';
  for(var i=0;i<prods.length;i++){
    var x=prods[i];
    var colors=['#4CAF50','#FF9800','#2196F3','#9C27B0','#F44336','#00BCD4','#FF5722','#607D8B','#795548','#8BC34A'];
    ...
    h+='<div class="shop-card" onclick="openPrd('+x.id+')" style="background:var(--card);border-radius:12px;overflow:hidden;cursor:pointer;position:relative">'+
      '<div style="aspect-ratio:1;background:linear-gradient(135deg,'+bg+',rgba(0,0,0,0.1));display:flex;align-items:center;justify-content:center;font-size:52px">'+emoji+'</div>'+
      '<div style="padding:8px 10px 10px">'+
      '<div style="font-size:13px;font-weight:600;line-height:1.3;margin-bottom:4px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">'+esc(x.name)+'</div>'+
      '<div style="display:flex;align-items:baseline;gap:4px">'+
      '<span style="font-size:18px;font-weight:700;color:var(--green)">¥'+x.price+'</span>'+
      (x.original_price&&x.original_price>x.price?'<span style="font-size:11px;color:#999;text-decoration:line-through">¥'+x.original_price+'</span>':'')+
      '</div>'+
      (disc>0?'<span style="display:inline-block;font-size:10px;background:#f44336;color:#fff;padding:1px 5px;border-radius:4px;margin-top:2px">省¥'+(x.original_price-x.price).toFixed(1)+'</span>':'')+
      '<div style="font-size:11px;color:var(--text2);margin-top:4px"><span>已售'+(x.sales_count||0)+'</span></div>'+
      '</div></div>';
  }
  h+='</div></div>';
  // 购物车浮标
  if(cartCount>0){
    h+='<div onclick="navigate(\'shop-cart\')" style="position:fixed;bottom:72px;right:16px;background:var(--green);color:#fff;width:52px;height:52px;border-radius:50%;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 16px rgba(76,175,80,.4);z-index:100;font-size:20px;cursor:pointer">'+
      '<i class="fa-solid fa-cart-shopping"></i>'+
      '<span style="position:absolute;top:-4px;right:-4px;background:#f44336;color:#fff;font-size:11px;min-width:18px;height:18px;border-radius:9px;display:flex;align-items:center;justify-content:center;font-weight:600">'+cartCount+'</span></div>';
  }
...
function openDiary(i){if(window._diaryList&&window._diaryList[i])navigate("diary-edit",{diary:window._diaryList[i]});}
function openRecipe(id){api("/recipes/"+id).then(function(r){navigate("recipe-detail",{recipe:r});}).catch(function(e){toast(e.message,"error");})}
...
async function renderBadges() {
  try {
    ...
    var html = '<div class="tea-badge-row">';
    for (var bi = 0; bi < earnedBadges.length; bi++) {
      html += '<div class="tea-badge" title="' + esc(earnedBadges[bi].description) + '"><span class="tea-badge-icon">' + (earnedBadges[bi].icon || "🏅") + '</span><span class="tea-badge-name">' + esc(earnedBadges[bi].name) + "</span></div>";
    }
    return html + "</div>";
  } catch (e) { return ""; }
}
...
function startBrew(teaId, teaName, temp, minutes) {
  ...
  var h = hd("☕ 冲泡引导");
  var b = '<div class="tea-brew-page" id="brew-page">';
  b += '<div class="tea-brew-name">' + esc(teaName) + "</div>";
  b += '<div class="tea-brew-temp">水温 ' + temp + "°C</div>";
  b += '<div class="tea-brew-timer" id="brew-timer">' + fmtTime(totalSec) + "</div>";
  b += '<div class="tea-brew-status" id="brew-status">准备开始</div>';
  b += '<div class="tea-brew-progress"><div class="tea-brew-bar" id="brew-bar" style="width:0%"></div></div>';
  b += '<div class="tea-brew-btns">';
  b += '<button class="btn btn-primary" id="brew-start-btn" onclick="brewStart(' + totalSec + ')">▶ 开始冲泡</button>';
  b += '<button class="btn btn-outline mt-2" id="brew-done-btn" style="display:none" onclick="brewDone(' + teaId + "," + JSON.stringify(teaName) + ')">☕ 喝完了</button>';
  b += "</div></div>";
  document.getElementById("app").innerHTML = h + b;
}

var brewInterval = null;
function brewStart(totalSec) {
  var btn = document.getElementById("brew-start-btn");
  if (btn) btn.style.display = "none";
  document.getElementById("brew-done-btn").style.display = "block";
  ...
  brewInterval = setInterval(function() {
    ...
    if (remaining <= 0) {
      ...
      brewInterval = null;
      document.getElementById("brew-timer").textContent = "00:00";
      document.getElementById("brew-status").textContent = "✅ 可以出汤啦！";
      document.getElementById("brew-bar").style.width = "100%";
      document.getElementById("brew-done-btn").textContent = "☕ 可以喝啦";
      return;
    }
    document.getElementById("brew-timer").textContent = fmtTime(remaining);
    document.getElementById("brew-bar").style.width = (100 - (remaining / totalSec) * 100) + "%";
    if (remaining <= 10) document.getElementById("brew-status").textContent = "即将出汤!";
  }, 1000);
}
...
// 完成冲泡反馈
// ═══════════════════════════════════════════════════
function brewDone(teaId, teaName) {
  if (brewInterval) { clearInterval(brewInterval); brewInterval = null; }
  var h = hd("☕ 反馈");
  var b = '<div class="tea-review-page">';
  b += '<div style="font-size:36px;margin-bottom:16px">😊</div>';
  b += '<div style="font-size:16px;font-weight:600;margin-bottom:12px">口感如何？</div>';
  b += '<div class="tea-score-btns" id="tea-score-btns">';
  for (var i = 1; i <= 5; i++) b += '<button class="tea-score-btn" onclick="selTeaScore(' + i + ')">' + ["😫","😐","😊","😄","🤩"][i-1] + '</button>';
  b += '</div>';
  b += '<div style="margin:16px 0;width:100%"><textarea id="tea-feeling" class="form-textarea" placeholder="身体感受（选填）..."></textarea></div>';
  b += '<button class="btn btn-primary btn-block btn-lg" onclick="submitTeaRecord(' + teaId + "," + JSON.stringify(teaName) + ')">提交记录</button>';
  b += "</div>";
  document.getElementById("app").innerHTML = h + b;
}

var teaScore = 0;
function selTeaScore(s) { teaScore = s; var btns = document.querySelectorAll(".tea-score-btn"); for (var i = 0; i < btns.length; i++) btns[i].classList.toggle("selected", i + 1 === s); }

async function submitTeaRecord(teaId, teaName) {
...
function getPopup() {
  ...
  if (!p) { p = document.createElement("div"); p.id = "popup"; p.style.cssText = "position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center"; document.body.appendChild(p); }
  return p;
}
...
function showTimeRule(id) {
  api("/tea/time-rules").then(function(rules) {
    ...
    if (!tr) return;
    var pop = getPopup();
    pop.innerHTML = "<div style=background:var(--card);padding:24px;border-radius:16px;width:90%;max-width:340px><h3 style=margin:0 0 8px>" + (tr.icon || "🫖") + " " + esc(tr.label) + "</h3><div style=font-size:13px;color:var(--text2);line-height:1.6;margin-bottom:16px>" + esc(tr.description) + '</div><button class="btn btn-sm" style=width:100%;background:var(--green);color:#fff;border:none;padding:10px;border-radius:8px onclick=closePopup()>知道了</button></div>';
...
function showTeaWiki() {
  api("/tea/products").then(function(teas) {
    ...
    html += '<button class="btn btn-sm" style=width:100%;margin-top:12px;background:var(--green);color:#fff;border:none;padding:10px;border-radius:8px onclick=closePopup()>关闭</button></div>';
    var pop = getPopup();
    pop.innerHTML = html;
    pop.style.display = "flex";
  }).catch(function() {});
...
// ── Demo Auto-Login ──
(function(){var p=new URLSearchParams(window.location.search);if(p.get('demo')==='1'){localStorage.removeItem('token');}})();
